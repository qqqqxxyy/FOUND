...
class FoundModel(nn.Module):
    ...
    def forward_step(self, batch, decoder=None, for_eval=False):
        ...
        # Forward pass
        with torch.no_grad():#vit 参数不变
            # Encoder forward pass
            # batch.shape: 50,3,224,224
            att = self.vit_encoder.get_last_selfattention(batch)
            # att.shape: 50,6,785,785
            # ipdb.set_trace()
            # Get decoder features; features comes from 'k'
